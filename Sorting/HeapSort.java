 ...
 * Rinse and repeat
 * 
 * Worst Case time complexity : O(n log n) as we are swapping n elements and at each iteration we fix heap whcih is O(log n)
 * After sorting the array it is no longer a heap and so we would not want to sort a heap that we want to keep reducing
 */
...
class Heap {
    ...
    public void sort(){
        ...
        for(int i = 0; i < lastHeapIndex; i ++){
            int temp = heap[0];//swap the root with the last item
            heap[0] = heap[lastHeapIndex - i];
            heap[lastHeapIndex - i] = temp;

            fixHeapBelow(0, lastHeapIndex - i - 1);//lastHeapIndex - i - 1 as we want to exclude where we have put the root
        }
    }
...
public class HeapSort {
    public static void main(String[] args) {
        ...
        heap.printHeap();//before sort
        
        heap.sort();

        heap.printHeap();//after sort
